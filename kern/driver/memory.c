...
// 内核页目录表地址,内核页目录在kernel镜像之后,之后通过calEndAddr函数计算出来
volatile uint32_t PageCataTableBase;

// 计算内核结束的地址,并向上以4k去整
static void calEndAddr(){
    extern char end[];
    PageCataTableBase = ROUNDUP((uint32_t)end, 4096);
}
...
static void* vaddr_user_get(uint32_t pg_cnt) {
...
// 在内核虚拟内存池中申请pg_cnt个虚拟页
// 成功则返回虚拟页的起始地址, 失败则返回NULL
static void* vaddr_kernel_get(uint32_t pg_cnt) {
    int vaddr_start = 0, bit_idx_start = -1;
    uint32_t cnt = 0;
...
// 得到虚拟地址vaddr对应的pte指针,也就是页表指针,即该虚拟地址在页表中的位置
...
// 得到虚拟地址vaddr对应的pde的指针,也就是页目录表指针,即该虚拟地址在页目录表中的位置
...
// 页表中添加虚拟地址_vaddr 与 物理地址_page_phyaddr的映射
...
void* malloc_user_page(uint32_t pg_cnt) {
...
***************************************************************/
    void* vaddr_start = vaddr_user_get(pg_cnt);
    if (vaddr_start == NULL) {
        return NULL;
    }
    ...
    while (cnt-- > 0) {
        void* page_phyaddr = palloc(&user_pool);
...
/***********   malloc_page的原理是三个动作的合成:   ***********
      ...
      3通过page_table_add将以上两步得到的虚拟地址和物理地址在页表中完成映射
***************************************************************/
    void* vaddr_start = vaddr_kernel_get(pg_cnt);
    if (vaddr_start == NULL) {
        return NULL;
    }
    uint32_t vaddr = (uint32_t)vaddr_start, cnt = pg_cnt;
...
//    lock_acquire(&kernel_pool.lock);
    ...
    void* vaddr =  malloc_kernel_page(pg_cnt);
    if (vaddr != NULL) {	   
        // 成功分配,将页框清0后返回
...
//    lock_acquire(&user_pool.lock);
    ...
    void* vaddr = malloc_user_page(pg_cnt);
    if (vaddr != NULL) {	   
        // 成功分配,将页框清0后返回
        memset(vaddr, 0, pg_cnt * PG_SIZE);
    }
...
int free_kernel_pages(uint32_t vaddr){
    
}


int free_user_pages(uint32_t vaddr){
...
static void mem_pool_init(uint32_t all_mem) {
    ...
    // uint32_t page_table_size = PG_SIZE * (1+ 254 + 2);	        
    ...
    // 已经使用的内存为页表起始地址+一个页目录表4KB+一个页表   
    uint32_t used_mem = PageCataTableBase + PGSIZE * 2;   
 ...
 *   ************************************************/
    ...
    // 虚拟地址的可用地址的起始地址为 物理可用地址的起始地址 + 3GB
    kernel_vaddr.vaddr_start = kernel_pool.phy_addr_start + 0xC0000000 ;
